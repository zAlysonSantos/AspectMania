...
class Home(id: EntityID<Int>) : IntEntity(id) {
    companion object : IntEntityClass<Home>(HomeTable) {
        fun newHome(owner: Player, home: String, location: Location): Home {
            return newHome(owner.name, home, location)
        }
        ...
        fun deleteHome(player: String, home: String) {
            findHome(player, home)?.delete();
        }
    ...
    var location by location(
            HomeTable.world,
            HomeTable.x, HomeTable.y, HomeTable.z,
            HomeTable.yaw, HomeTable.pitch
    )
